...
public class EncodeDecode {
	...
	void encode(String fName,String bfName, String freqWts, boolean optimize) {
		if(ErrorCheck(fName, true) && ErrorCheck(bfName, false)) {
			String code = "";
			...
			huffUtil.createHuffmanCodes(huffUtil.getTreeRoot(), code, 0);
	...
	private void executeEncode(File inFile, File binFile) {
		...
		BufferedOutputStream bups = fio.openBufferedOutputStream(binFile);
		int c = 0;
		try {
			while((c = br.read()) != -1) {
				binStr = binStr.concat(encodeMap[c]);
				if(binStr.length() > 8) {
					binStr = binUtil.writeBinString(bups, binStr);
				}
			}
	...
	void decode(String bfName, String ofName, String freqWts,boolean optimize)  {
		if(ErrorCheck(bfName, true) && ErrorCheck(ofName, false)) {
	...
	/**
	 * Error check.
	 * Error checks read and write files to see if they are valid
	 ...
	 * @param fName  
	 */
	private boolean ErrorCheck(String fName,boolean read) {
		if(read) {
			if(fio.getFileStatus(fio.getFileHandle(fName), read) == MyFileIO.FILE_OK) {
				if(hca.issueAlert(HuffAlerts.CONFIRM, "confirm " , "confirm")) {
					return true;
				}
				return false;
			}
			hca.issueAlert(HuffAlerts.INPUT, "input error", "inpit error");
			return false;
		}
		else {
			if(fio.getFileStatus(fio.getFileHandle(fName), read) == MyFileIO.FILE_OK) {
			...
			hca.issueAlert(HuffAlerts.INPUT, "output error", "output error");
			return false;
		}
	}
	...
	private void executeDecode(File binFile, File outFile) throws IOException {
		...
		BufferedOutputStream bups = fio.openBufferedOutputStream(outFile);
		while((c = br.read()) != -1) {
			binStr+= binUtil.convBinToStr(c);
			while((crt = huffUtil.decodeString(binStr)) != -1){
				...
				if(crt == 0) {
					...
					binStr = binStr.substring(encodeMap[crt].length());
					bups.write(crt);
